# LogicLab

[English](README.en.md) | 简体中文

一个数字电路沙盒：对标《图灵完备（Turing Complete）》的沙盒模式，但按「半游戏半专业工具」的定位做得更专业一些。

> **⚠️ 注意**：本项目将长期处于测试阶段，可预见的时间内不会发布正式版（版本以 **Checkpoint 检查点**的形式推进）。迭代过程中可能出现**破坏性变更**，且**不保证向后兼容**——升级后旧工程文件或电路可能无法继续使用。请勿用于生产环境。

- **纯 Rust 内核** + Godot 4 外壳（GDExtension 桥接）
- **tick 制离散仿真**：所有组件输出一律延迟 1 tick，信号沿导线逐拍传播看得见
- **层次封装**：选中一片电路就能封装成自定义元件，双击实例进入内部继续改（TC 的核心玩法）
- **波形查看 + VCD 导出**：鼠标停在导线上按 W 就能观察，录完可直接丢给 GTKWave
- **虚拟外设**：七段数码管、内存映射点阵屏（画面状态住在内核里，撤销/读档会自动复位）
- **为大体量设计而做**：一万个元件每拍仿真约 0.4 ms，脏标记让静默电路零求值
- **x64 与 ARM64 双架构**，单文件免安装
- 附带一个 headless CLI，**不依赖 Godot** 也能跑仿真

## 快速开始

### 直接运行

到 [Releases](../../releases) 下载对应架构的单文件 exe，双击即可。

### 从源码构建

前置：Rust 稳定版工具链、Godot 4.7.x、MSVC（Windows）。

```bash
# 1) 内核与命令行（完全不需要 Godot）
cargo test
cargo run --release -p logiclab-cli -- bench      # 万级规模性能基准
cargo run -p logiclab-cli -- demo                 # 半加器真值表 + 移位寄存器波形
cargo run -p logiclab-cli -- example out.json     # 生成示例工程
cargo run -p logiclab-cli -- components           # 列出全部内置组件

# 2) GDExtension 桥
cd bridge
cargo build --release --target x86_64-pc-windows-msvc
cargo build --release --target aarch64-pc-windows-msvc
# 把产物复制到 godot/ 下：
#   x86_64  -> godot/logiclab_bridge.dll
#   aarch64 -> godot/logiclab_bridge_arm64.dll

# 3) 运行
godot --path godot
```

### 回归测试

```bash
# headless 也会加载 GDExtension，所以这是最快的全链路验证
godot --headless --path godot --script res://tests/smoke.gd
```

## 操作

| 操作 | 方式 |
|---|---|
| 载入示例 | 顶部「图纸库」左侧的示例下拉 |
| 放置元件 / 子电路 | 左侧库中点击，再点画布；子电路从顶部「图纸库」挑选 |
| 连线 | 从一个引脚拖到另一个引脚（自动避让，不会误连） |
| 选中 / 拖动 | 左键点元件 / 按住拖动（多选可一起拖） |
| 多选 / 框选 | Shift + 左键点选 / 在空白处拖出框 |
| 开关翻转 | 单击开关元件 |
| 旋转 / 删除 | 选中后按 R / Delete |
| 进入子电路 | 双击实例，或选中后按 E |
| 返回上一层 | 按 B，或点顶部的面包屑 |
| 新建 / 改名 / 删除图纸 | 顶部导航条上的按钮 |
| 封装选区 | 选中若干元件 → 顶部「封装选区」→ 起个名字 |
| 观察波形 | 鼠标停在导线上按 W；顶部「波形」按钮开面板 |
| 平移 / 缩放 / 居中 | 右键或中键拖动 / 滚轮 / C |
| 单步 / 运行 | 空格 / 底部按钮（右侧滑块调速） |
| 改参数 | 选中元件后在右侧面板改 |
| 设计规则检查 | 左侧按钮（多驱动 / 位宽 / 悬空 / 组合环 / 循环引用） |

## 架构

```
Godot 壳（GDScript）    交互 / 自绘渲染 / 面板 —— 不硬编码任何组件知识
      ↓ 调方法、拿扁平数组
bridge（gdext，薄）     命令翻译 + CircuitView → PackedInt64Array
      ↓ 纯 Rust 调用
core（零引擎依赖）      数据模型 / 层次展开 / tick 引擎 / 网表推导 / 序列化 / 视图 / DRC / 示例
      ↓
cli（headless）         同一套 core 的非 Godot 消费者
```

**铁律：core 的依赖树里没有任何引擎类型。** 所以 `cargo test` 不需要 Godot，
将来换壳（或干脆去掉 Godot）时 core 一行不改。

### core 模块

| 文件 | 职责 |
|---|---|
| `values.rs` | 位宽 {1,4,8,16,32}、位级未知标记、wrap 算术 |
| `defs.rs` | **组件知识的唯一数据源**：种类 / 参数 / 引脚 / 求值 / 可编辑参数描述 |
| `board.rs` | 编辑期模型、网表推导（区间合并索引）、自动走线（网格 Dijkstra）、命中测试 |
| `elaborate.rs` | 层次展开：多图纸 → 一张扁平网表；接口排序、网络合并、自引用截断 |
| `engine.rs` | 双缓冲 tick、脏传播、CSR 邻接 |
| `view.rs` | 壳无关的 `CircuitView`（换壳只需重写一个渲染器） |
| `save.rs` | 工程 JSON + schema 版本（core 不碰文件系统） |
| `drc.rs` | 设计规则检查（全工程 + 循环引用） |
| `session.rs` | 编辑 / 仿真的唯一门面：撤销、编辑即重置、图纸导航、封装、波形 |
| `examples.rs` | 内置示例电路（含层次示例） |

## 层次（自定义元件）

一张图纸就是一个子电路定义，里面的 `input_pin` / `output_pin` 是它的接口：

1. 先在根图纸把电路搭出来、跑通；
2. 框选或 Shift 点选那一片；
3. 点「封装选区」起个名字——跨边界的连线会**自动转成接口并重新接回**；
4. 之后它就是一个普通元件，可以反复实例化、可以再被封装进更大的电路。

接口引脚按**元件名排序**（同名按实例索引），所以同一张图纸每次得到同样的引脚顺序。
仿真开始前，整棵层次树被压成一张扁平网表：外壳不产生运行时组件，外壳引脚与接口引脚
并成同一个网络。**热路径完全不知道层次存在**，一万个元件里有多少层都不影响每拍开销。

## 仿真语义

- 所有组件输出**一律延迟 1 tick**（含触发器：时钟沿在 t 被读到，输出在 t+1 出现）
- 导线零延迟、同一网络同值
- 两阶段更新：先全部求值到暂存区，再原子提交，结果与遍历顺序无关
- 未连接的输入读作 0
- 算术一律 wrap 回绕，逻辑层全整数、不出现浮点

注意：**组合反馈（如 SR 锁存器）能仿真但会被 DRC 报错**。反馈本身合法，只要环上有时序元件；
纯组合环对应真实硬件里的振荡，也是 Verilog 导出的硬伤，所以按错误处理。

## 性能

| 指标 | 数值 |
|---|---|
| 10001 个组件，每拍全部活动 | ~0.41 ms/拍 |
| 万级电路单次编辑（含层次展开与引擎重建） | ~13.5 ms |
| 静默电路每拍求值组件数 | 0 |

## 路线图

按[规格书 v4](docs/logic-sandbox-spec-v4.md) 的里程碑推进，逐项状态见 [docs/ROADMAP.md](docs/ROADMAP.md)。

| 里程碑 | 范围 | 状态 |
|---|---|---|
| **M0** 纯 Rust 内核 | 数据模型 / tick 引擎 / 网表推导 / 存档 / 撤销 / CLI | ✅ 完成 |
| **M1** Godot 最小壳 | 编辑交互 / 自绘渲染 / 调速 / 注释 | ✅ 完成 |
| **M2** 完整编辑器 + 全组件库 | 全组件 / 封装 / 网络标签 / 多图纸 / DRC / 万元件性能 | ✅ 完成 |
| **M3** 专业四件套 + checkpoint | 波形与 VCD ✅ · 关键路径 ✅ · 测试台 ⬜ · CLI 回归 ⬜ · checkpoint ⬜ | 🚧 进行中 |
| **M4** Verilog 导出 | 结构化导出 / 单时钟约束 / 外设 black-box；验收 = iverilog 仿真 + Yosys 综合双通过 | ⬜ 计划中 |
| **M5** 造机与外设 | Keyboard / UART / 汇编器 + ISA / ROM 烧录 / 参考 CPU 教程 | ⬜ 计划中 |
| **M6+** | FSM 编辑器、三态总线、真值表双向、SVG 导出、i18n | ⬜ 远期 |

最终目标是把规格 §15 的七条验收标准全部打勾。其中最硬的两条是：**用自建 CPU + UART 跑通一段
含条件跳转的真实程序**，以及**任意电路导出的 Verilog 能被 iverilog 与 Yosys 双双接受**。

### 平台支持

| 平台 | 状态 | 说明 |
|---|---|---|
| Windows x64 / ARM64 | ✅ 已支持 | 单文件免安装，见 [Releases](../../releases) |
| Linux x64 / ARM64 | 🔜 计划中 | 内核与 CLI 本就是跨平台的，主要工作量在单文件封装与 Godot 导出这一层；排期取决于实际需求 |
| macOS | ⏸ 暂无计划 | 缺少可持续的验证环境。一个无法自行确认可用性的平台，先不做承诺 |

架构上留了余地：`core` 与 `cli` 是纯 Rust、不依赖任何引擎类型，换平台不需要动内核；
真正需要重做的只有打包与导出流程。所以上表是排期问题，不是能力边界。

### 关于是否长期依赖 Godot

`core` 从第一天起就不含任何引擎类型，`view.rs` 也只产出与壳无关的呈现数据——这条约束本就是为了
"将来换壳时 core 一行不改"。所以脱离 Godot 在架构上可行，只是不在当前的优先级里：

| 路线 | 收益 | 代价 |
|---|---|---|
| 自编译裁剪 Godot（关掉 3D / 物理 / 导航 / XR 等模块） | 单文件 44 MB → 约 25 MB | 需要 scons + Python 工具链，两个架构各编一次；Godot 升级要跟着重编 |
| 换成纯 Rust 壳（winit + 自绘 UI） | 单文件约 5–10 MB，构建完全走 cargo | 要重写全部 UI 层与交互；`core` / `cli` / `view` 原样带走 |

判断是**先不折腾**：当前的瓶颈是功能完整度（测试台、Verilog 导出、造机玩法），不是体积或依赖。
等 M5 收尾、P1 功能开始排期时重新评估——若那时体积或分发体验真的成了障碍，上面两条路线都还开着。

## 关于 AI 辅助开发

本项目**大量使用人工智能辅助开发**，这里如实说明分工：

| 环节 | 由谁主导 |
|---|---|
| 产品定位与规格 | 人（性能优先、ARM64 必须能跑、以 TC 沙盒为平替、Checkpoint 版本策略） |
| 架构设计与决策 | 人机讨论后由人拍板（core 与壳彻底分离、编辑即重置、层次展开放在编辑期） |
| 代码实现 | AI 编码 agent（Rust 内核、GDExtension 桥、GDScript 外壳、测试与文档） |
| 验收 | 可执行的测试与基准（core 单元测试、headless 冒烟、性能基准跑绿） |

所有测试与性能基准都随仓库提供，跑一遍就能看到实际结果。

## 许可

[MIT](LICENSE) © 2026 Orlean-Crispy
